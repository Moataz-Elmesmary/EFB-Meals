import i18n from 'i18next';
import { initReactI18next } from 'react-i18next';

const resources = {
  en: {
    translation: {
      brandSub: 'Smart Kitchen Requests',
      navRequest: 'New Request',
      navKitchen: 'Kitchen',
      navReports: 'Reports',
      navMine: 'My Requests',
      myTitle: 'My Requests',
      mySub: 'Track the status of everything you ordered',
      myEmpty: 'No requests yet',
      myEmptySub: 'Your meal orders will show here.',
      reportsTitle: 'Reports',
      reportsSub: 'All meal requests, budgets and attachments',
      totalBudget: 'Total budget',
      search: 'Search…',
      allStatuses: 'All statuses',
      allDepartments: 'All departments',
      exportCsv: 'Export CSV',
      download: 'Download',
      colRequester: 'Requester',
      colMeal: 'Meal',
      colStatus: 'Status',
      colBudget: 'Budget',
      colAttachment: 'Attachment',
      login: 'Sign in with Microsoft',
      logout: 'Sign out',
      loggedInAs: 'Signed in as',

      // login gate
      loginWelcome: 'Welcome to EFB Meals',
      loginSub: 'Sign in to order delicious meals for you & your team.',
      loginSecure: '🔒 Secured by your organization account',
      demoNote: 'Demo mode — enter your work email to continue.',
      enter: 'Enter',
      prep: 'Setting the table…',

      // hero
      eyebrow: '🍴 Fresh meals, made simple',
      heroTitlePre: 'Delicious meals for you & your team,',
      heroTitleAccent: 'just a tap away',
      heroText: 'Pick from a chef-crafted menu or describe exactly what you crave. Fresh and fast.',
      ctaOrder: 'Order now',
      ctaMenu: 'Browse menu',
      ctaHow: 'How it works',
      statMeals: 'Menu items',
      statFast: 'Fast & fresh',
      statBilingual: 'Arabic & English',

      // marquee
      m1: 'Char-grilled', m2: 'Fresh salads', m3: 'Stone-baked', m4: 'Home-style', m5: 'Sweet treats', m6: 'Hot & fresh',

      // how it works
      howTitle: 'Order in 3 easy steps',
      howSub: 'Pick, send, enjoy — that\'s it.',
      step1Title: 'Pick your meal',
      step1Text: 'Choose from a chef-crafted menu or describe exactly what your team is craving.',
      step2Title: 'The kitchen cooks',
      step2Text: 'Our kitchen prepares everything fresh, in the right quantities, for your whole team.',
      step3Title: 'Delivered fresh',
      step3Text: 'Your meals arrive hot and on time, ready to enjoy together.',

      // menu
      menuTitle: 'Today\'s Menu',
      menuSub: 'Pick a dish or switch to a special request',
      modeMenu: 'From the menu',
      modeSpecial: 'Special request',
      cartTitle: 'Your order',
      orderDetails: 'Order details',
      typeLabel: 'Request type',
      type_supervisors: 'Supervisors',
      type_event: 'Event',
      type_meeting: 'Meeting',
      classificationLabel: 'Classification',
      class_hot: 'Hot meal',
      class_ready: 'Ready-to-eat',
      locationLabel: 'Location',
      locationPlaceholder: 'e.g. 3rd floor, Hall B',
      choose: '— choose —',
      chooseType: 'Please choose a request type.',
      chooseDept: 'Please choose a department.',
      suggestedItems: 'Suggested items',
      suggestedHint: 'suggestions — the kitchen sets the final items',
      suggestedByRequester: 'Suggested by requester',
      finalItems: 'Final items (recorded)',
      budgetSection: 'Budget',
      kitchenNeedsItems: 'Add the items to record for this order',
      peopleShort: 'People',
      searchHint: 'searchable',
      itemNameLabel: 'Item name',
      descriptionLabel: 'Description',
      qtyLabel: 'Qty',
      addFromMenu: 'Add from menu',
      pickItems: 'Pick items',
      noItems: 'No items found',
      done: 'Done', next: 'Next', back: 'Back', noneYet: 'None', notSetYet: 'Not set yet',
      cartEmpty: 'Your cart is empty — pick from the menu above',
      addSpecialLine: 'Add to order',
      specialCardTitle: 'Want something not on the menu?',
      specialCardSub: 'Order anything you like — any quantity. Just type it below.',
      specialLinePlaceholder: 'e.g. 10 club sandwiches, 2 veg platters, no nuts…',
      addBtn: 'Add',

      // form
      formTitle: 'Create your meal request',
      formSub: 'Tell us who it\'s for and how many people',
      orderingAs: 'Ordering as',
      hello: 'Hi,',
      timeLabel: 'Delivery time',
      urgentHint: 'Ordered for today — marked URGENT',
      urgent: 'Urgent',
      notesLabel: 'Notes / comment',
      notesPlaceholder: 'Anything to add to the order…',
      optional: 'optional',
      reorder: 'Order again',
      nameLabel: 'Requester name',
      emailLabel: 'Email address',
      deptLabel: 'Department',
      deptPlaceholder: 'e.g. Operations',
      peopleLabel: 'Number of people',
      dateLabel: 'Needed on',
      selectedMeal: 'Selected meal',
      noMealSelected: 'No meal selected yet — pick one above',
      specialLabel: 'Describe your special request',
      specialPlaceholder: 'e.g. 20 mixed sandwiches, 2 vegetarian platters, no nuts...',
      submit: 'Send request',
      submitting: 'Sending...',
      requestSent: 'Request sent to the kitchen! 🎉',
      error: 'Something went wrong. Please try again.',
      pickMealOrSpecial: 'Pick a meal or write a special request.',

      // kitchen
      kitchenTitle: 'Kitchen Dashboard',
      kitchenSub: 'Every incoming order — review, set the budget, and approve',
      filtering: 'Filtering',
      clearFilter: 'Clear',
      allF: 'All', sortNewest: 'Newest first', sortOldest: 'Oldest first', sortUrgent: 'Urgent first',
      kpiNew: 'New',
      kpiBudget: 'Budgeted',
      kpiReady: 'Completed',
      kpiAll: 'Total',
      refresh: 'Refresh',
      people: 'people',
      special: 'Special request',
      createBudget: 'Create budget',
      viewAttachment: 'View attachment',
      markReady: 'Mark ready',
      sendingSap: 'Processing...',
      noRequests: 'No requests yet',
      noRequestsSub: 'New meal requests will appear here instantly.',

      // budget modal
      budgetTitle: 'Create budget request',
      budgetSub: 'Amount and an attachment are required',
      amountLabel: 'Budget amount',
      currencyLabel: 'Currency',
      vendorLabel: 'Vendor (optional)',
      notesLabel: 'Notes (optional)',
      attachmentLabel: 'Attachment (required)',
      chooseFile: 'Click to upload a document',
      fileSelected: 'File ready',
      cancel: 'Cancel',
      save: 'Save budget',
      saving: 'Saving...',
      budgetSaved: 'Budget saved & requester notified',
      attachmentRequired: 'Please attach a document.',
      amountRequired: 'Please enter a valid amount.',

      // statuses
      status_requested: 'New · review',
      status_budget_set: 'Budget set',
      status_budget_uploaded: 'Budget · review',
      status_rejected: 'Rejected',
      status_ready_for_sap: 'Completed',
      setBudget: 'Approve & set budget',
      setBudgetTitle: 'Approve order & set budget',
      setBudgetSub: 'Enter the required budget; the requester will upload the document',
      setBudgetSend: 'Set budget',
      rejectOrder: 'Reject order',
      rejectOrderPrompt: 'Reason for rejecting this order:',
      requiredBudget: 'Required budget',
      orderWord: 'Order',
      totalQty: 'Total qty',
      asap: 'ASAP',
      awaitingUpload: 'Awaiting budget document from requester',
      viewBudget: 'View budget',
      approve: 'Approve',
      reject: 'Reject',
      addNote: 'Add note',
      rejectPrompt: 'Reason for rejection:',
      notePrompt: 'Note to the requester:',
      kpiAwaiting: 'Awaiting budget',
      kpiToReview: 'To review',
      uploadBudgetTitle: 'Upload the budget (PDF) for this order',
      sendBudget: 'Send budget',
      budgetSent: 'Budget sent successfully!',
      kitchenSays: 'Kitchen',
      rejectedReason: 'Rejected',

      footer: 'EFB Meals · Designed for a fast kitchen workflow.'
    }
  },
  ar: {
    translation: {
      brandSub: 'طلبات المطبخ الذكي',
      navRequest: 'طلب جديد',
      navKitchen: 'المطبخ',
      navReports: 'التقارير',
      navMine: 'طلباتي',
      myTitle: 'طلباتي',
      mySub: 'تابع حالة كل اللي طلبته',
      myEmpty: 'لا توجد طلبات بعد',
      myEmptySub: 'طلبات وجباتك هتظهر هنا.',
      reportsTitle: 'التقارير',
      reportsSub: 'كل طلبات الوجبات والميزانيات والمرفقات',
      totalBudget: 'إجمالي الميزانية',
      search: 'بحث…',
      allStatuses: 'كل الحالات',
      allDepartments: 'كل الإدارات',
      exportCsv: 'تصدير CSV',
      download: 'تحميل',
      colRequester: 'مقدّم الطلب',
      colMeal: 'الوجبة',
      colStatus: 'الحالة',
      colBudget: 'الميزانية',
      colAttachment: 'المرفق',
      login: 'دخول بحساب مايكروسوفت',
      logout: 'تسجيل الخروج',
      loggedInAs: 'مسجّل دخول باسم',

      // login gate
      loginWelcome: 'أهلاً بك في EFB Meals',
      loginSub: 'سجّل الدخول عشان تطلب وجبات شهية ليك ولفريقك.',
      loginSecure: '🔒 مؤمَّن بحساب مؤسستك',
      demoNote: 'وضع تجريبي — اكتب إيميل العمل عشان تكمّل.',
      enter: 'دخول',
      prep: 'بنجهّز السفرة…',

      eyebrow: '🍴 وجبات طازة وببساطة',
      heroTitlePre: 'وجبات شهية ليك ولفريقك،',
      heroTitleAccent: 'على بُعد ضغطة',
      heroText: 'اختر من منيو من إعداد الشيف، أو اكتب بالظبط اللي نفسك فيه. طازة وسريعة.',
      ctaOrder: 'اطلب دلوقتي',
      ctaMenu: 'تصفّح المنيو',
      ctaHow: 'إزاي بيشتغل',
      statMeals: 'صنف في المنيو',
      statFast: 'سريع وطازج',
      statBilingual: 'عربي وإنجليزي',

      // marquee
      m1: 'مشوي على الفحم', m2: 'سلطات طازة', m3: 'على الحطب', m4: 'زي البيت', m5: 'حلويات', m6: 'سخن وطازج',

      // how it works
      howTitle: 'اطلب في ٣ خطوات',
      howSub: 'اختار، ابعت، استمتع — وخلاص.',
      step1Title: 'اختر وجبتك',
      step1Text: 'اختر من منيو من إعداد الشيف، أو اكتب بالظبط اللي نفس فريقك فيه.',
      step2Title: 'المطبخ بيجهّز',
      step2Text: 'مطبخنا بيحضّر كل حاجة طازة، بالكميات المظبوطة، لفريقك كله.',
      step3Title: 'يوصل طازج',
      step3Text: 'وجباتك توصل سخنة وفي وقتها، جاهزة تتستمتعوا بيها سوا.',

      menuTitle: 'منيو النهاردة',
      menuSub: 'اختر طبق أو حوّل لطلب خاص',
      modeMenu: 'من المنيو',
      modeSpecial: 'طلب خاص',
      cartTitle: 'طلبك',
      orderDetails: 'تفاصيل الطلب',
      typeLabel: 'نوع الطلب',
      type_supervisors: 'مشرفين',
      type_event: 'فعالية',
      type_meeting: 'اجتماع',
      classificationLabel: 'التصنيف',
      class_hot: 'وجبة ساخنة',
      class_ready: 'جاهزة (Ready-to-eat)',
      locationLabel: 'المكان',
      locationPlaceholder: 'مثال: الدور الثالث، قاعة B',
      choose: '— اختر —',
      chooseType: 'برجاء اختيار نوع الطلب.',
      chooseDept: 'برجاء اختيار الإدارة.',
      suggestedItems: 'الأصناف المقترحة',
      suggestedHint: 'مقترحات — المطبخ بيحدد الأصناف النهائية',
      suggestedByRequester: 'اقتراح مقدّم الطلب',
      finalItems: 'الأصناف المعتمدة (المسجّلة)',
      budgetSection: 'الموازنة',
      kitchenNeedsItems: 'أضف الأصناف اللي هتتسجّل للطلب',
      peopleShort: 'الأشخاص',
      searchHint: 'بحث',
      itemNameLabel: 'اسم الصنف',
      descriptionLabel: 'الوصف',
      qtyLabel: 'الكمية',
      addFromMenu: 'أضف من القائمة',
      pickItems: 'اختر الأصناف',
      noItems: 'مفيش أصناف',
      done: 'تم', next: 'التالي', back: 'رجوع', noneYet: 'لا يوجد', notSetYet: 'لم تُحدَّد بعد',
      cartEmpty: 'سلتك فاضية — اختر من المنيو فوق',
      addSpecialLine: 'أضف للطلب',
      specialCardTitle: 'عايز حاجة مش في المنيو؟',
      specialCardSub: 'اطلب أي حاجة تحبها بأي كمية — اكتبها تحت وضيفها لطلبك.',
      specialLinePlaceholder: 'مثال: ١٠ ساندويتش، طبقين نباتي، من غير مكسرات…',
      addBtn: 'أضف',

      formTitle: 'أنشئ طلب وجبتك',
      formSub: 'قولنا الطلب لمين وعدد كام فرد',
      orderingAs: 'بتطلب باسم',
      hello: 'أهلاً',
      timeLabel: 'وقت الاستلام',
      urgentHint: 'مطلوب النهاردة — هيتسجّل عاجل',
      urgent: 'عاجل',
      notesLabel: 'ملاحظات / تعليق',
      notesPlaceholder: 'أي حاجة تحب تضيفها على الأوردر…',
      optional: 'اختياري',
      reorder: 'اطلب تاني',
      nameLabel: 'اسم مقدّم الطلب',
      emailLabel: 'البريد الإلكتروني',
      deptLabel: 'الإدارة',
      deptPlaceholder: 'مثال: العمليات',
      peopleLabel: 'عدد الأفراد',
      dateLabel: 'التاريخ المطلوب',
      selectedMeal: 'الوجبة المختارة',
      noMealSelected: 'لسه ما اخترتش وجبة — اختر من فوق',
      specialLabel: 'اكتب طلبك الخاص',
      specialPlaceholder: 'مثال: ٢٠ ساندويتش متنوع، طبقين نباتي، من غير مكسرات...',
      submit: 'أرسل الطلب',
      submitting: 'جاري الإرسال...',
      requestSent: 'تم إرسال الطلب للمطبخ! 🎉',
      error: 'حصل خطأ. حاول تاني.',
      pickMealOrSpecial: 'اختر وجبة أو اكتب طلب خاص.',

      kitchenTitle: 'لوحة المطبخ',
      kitchenSub: 'كل الطلبات الواردة — راجِع، حدّد الموازنة، واعتمد',
      filtering: 'تصفية',
      clearFilter: 'مسح',
      allF: 'الكل', sortNewest: 'الأحدث أولاً', sortOldest: 'الأقدم أولاً', sortUrgent: 'العاجل أولاً',
      kpiNew: 'جديد',
      kpiBudget: 'تم تسعيره',
      kpiReady: 'مكتمل',
      kpiAll: 'الإجمالي',
      refresh: 'تحديث',
      people: 'فرد',
      special: 'طلب خاص',
      createBudget: 'إنشاء ميزانية',
      viewAttachment: 'عرض المرفق',
      markReady: 'تحديد كجاهز',
      sendingSap: 'جاري المعالجة...',
      noRequests: 'لا توجد طلبات بعد',
      noRequestsSub: 'الطلبات الجديدة هتظهر هنا فوراً.',

      budgetTitle: 'إنشاء طلب ميزانية',
      budgetSub: 'المبلغ والمرفق إجباريان',
      amountLabel: 'مبلغ الميزانية',
      currencyLabel: 'العملة',
      vendorLabel: 'المورّد (اختياري)',
      notesLabel: 'ملاحظات (اختياري)',
      attachmentLabel: 'المرفق (إجباري)',
      chooseFile: 'اضغط لرفع مستند',
      fileSelected: 'الملف جاهز',
      cancel: 'إلغاء',
      save: 'حفظ الميزانية',
      saving: 'جاري الحفظ...',
      budgetSaved: 'تم حفظ الميزانية وإخطار مقدّم الطلب',
      attachmentRequired: 'برجاء إرفاق مستند.',
      amountRequired: 'برجاء إدخال مبلغ صحيح.',

      status_requested: 'جديد · للمراجعة',
      status_budget_set: 'تم تحديد الموازنة',
      status_budget_uploaded: 'موازنة · للمراجعة',
      status_rejected: 'مرفوض',
      status_ready_for_sap: 'مكتمل',
      setBudget: 'موافقة وتحديد الموازنة',
      setBudgetTitle: 'موافقة على الطلب وتحديد الموازنة',
      setBudgetSub: 'اكتب الموازنة المطلوبة؛ مقدّم الطلب هيرفع المستند',
      setBudgetSend: 'تحديد الموازنة',
      rejectOrder: 'رفض الطلب',
      rejectOrderPrompt: 'سبب رفض الطلب:',
      requiredBudget: 'الموازنة المطلوبة',
      orderWord: 'طلب',
      totalQty: 'إجمالي الكمية',
      asap: 'في أقرب وقت',
      awaitingUpload: 'بانتظار مستند الموازنة من مقدّم الطلب',
      viewBudget: 'عرض الموازنة',
      approve: 'موافقة',
      reject: 'رفض',
      addNote: 'إضافة ملاحظة',
      rejectPrompt: 'سبب الرفض:',
      notePrompt: 'ملاحظة لمقدّم الطلب:',
      kpiAwaiting: 'بانتظار موازنة',
      kpiToReview: 'للمراجعة',
      uploadBudgetTitle: 'ارفع موازنة الطلب (PDF)',
      sendBudget: 'إرسال الموازنة',
      budgetSent: 'تم إرسال الموازنة بنجاح!',
      kitchenSays: 'المطبخ',
      rejectedReason: 'سبب الرفض',

      footer: 'EFB Meals · مصمَّم لسير عمل مطبخ سريع.'
    }
  }
};

i18n.use(initReactI18next).init({
  resources,
  lng: 'ar',
  fallbackLng: 'en',
  interpolation: { escapeValue: false }
});

export default i18n;
